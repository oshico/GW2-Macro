...
struct MousePosition {
    ...
    MousePosition(const int _x, const int _y, const EPositionType type = EPositionType::Absolute)
...
struct KeyAction {
    ...
    KeyAction(const EGameBinds bind, const bool down, const int delay = 0)
    ...
    KeyAction(const EMouseButton button, const bool down, const int delay = 0)
        : inputType(EInputType::MouseButton), gameBind(GB_SkillWeapon1),
          mouseButton(button),
          isKeyDown(down), moveBeforeClick(false), delayMs(delay) {
    }

    KeyAction(const EMouseButton button, const bool down, const MousePosition pos, const int delay = 0)
        : inputType(EInputType::MouseButton), gameBind(GB_SkillWeapon1),
          mouseButton(button), mousePosition(pos),
          isKeyDown(down), moveBeforeClick(true), delayMs(delay) {
    }

    explicit KeyAction(const MousePosition pos, const int delay = 0)
